...
> boards :: Puzzle -> [[Int]]
> boards p = map elems (reverse $ brds p)
>   where
...
> dias :: [[Int]] -> [Diagram B R2]
> dias ns = map (boardDia . length $ ns) ns 

> times :: Int -> [Int]
> times n = replicate (n-1) 100 ++ [300]

> gifs :: [[Int]] -> [(Diagram B R2, Int)]
> gifs ns = zip (dias ns) (times . length $ ns)
...
> totalDist b n = sum [manhattan (b ! (i, j)) n i j | i <- [0..n-1], j <- [0..n-1]]

 Create a start state from a list of tiles.

> mkPuzzle :: Int -> [Int] -> Puzzle
> mkPuzzle n xs = Puzzle b d n z 0 Nothing
>   where
